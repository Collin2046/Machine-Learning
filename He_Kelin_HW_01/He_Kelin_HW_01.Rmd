---
title: "INFSCI 2595 Fall 2021 Homework: 01"
subtitle: "Assigned September 1, 2021; Due: September 8, 2021"
author: "Kelin He"
date: "Submission time: September 8, 2021 at 11:00PM EST"
...
If you need help with understanding the R syntax please see the [R4DS book](https://r4ds.had.co.nz/) and/or the R tutorial videos and demos available on the [Canvas site](https://canvas.pitt.edu/courses/114820/pages/introduction-to-r-tutorials).  
...
```{r, solution_02b}
 diamonds %>% glimpse() %>% ggplot(mapping=aes(x=color,y=price))+geom_boxplot()
```

Discuss your conclusions here.  
With colors vary from D-J,the prices of median and maximum obviously go up.
...
```{r, solution_02c}
diamonds %>% glimpse() %>% ggplot(mapping=aes(x=color,y=price))+geom_boxplot()+facet_wrap(~cut)
```
...
```{r, solution_02d}
library(tidyverse)
diamonds %>% glimpse() %>% ggplot(mapping=aes(x=color,y=price))+geom_boxplot()+facet_grid(clarity~cut)
```
...
```{r, solution_02e}
#diamonds %>% glimpse() %>% ggplot(mapping=aes(x=color,y=price))+geom_boxplot(data=diamonds,fill=carat)+facet_grid(clarity~cut)
ggplot(data=diamonds)+geom_boxplot(mapping=aes(x=color,y=price,fill=carat>median(carat)))+facet_grid(clarity~cut)+theme(legend.position = "top")
```
...
```{r, solution_02f}
modifyDS = filter(diamonds,cut == "Ideal",clarity == "IF")
ggplot(data=modifyDS)+geom_boxplot(mapping=aes(x=color,y=price,fill=carat>median(diamonds$carat)))+facet_grid(clarity~cut)+theme(legend.position = "top")
```
...
According to result shown above, diamonds which have carat over median are more likely sold in high price compared with diamonds with small carat below median. However, the prices are not very influenced by the impact of color. For diamonds which have carat over median, the median and maximum price are fluctuant, not continuously go up, with colors vary from D-J.For diamonds which have small carat below median. The median and maximum prices of different colors' diamonds are nearly same, except for diamonds with D color.
...
```{r, solution_03d}
mpg %>% ggplot(mapping=aes(x=displ,y=hwy))+geom_point()+geom_smooth(method=lm,formula=y~x)
```
...
```{r, solution_03e}
mpg %>% ggplot(mapping=aes(x=displ,y=hwy))+geom_point()+geom_smooth(aes(color=drv),method=lm,formula=y~x)
```
...
```{r, solution_03f}
mpg %>% ggplot(mapping=aes(x=displ,y=hwy))+geom_point()+geom_smooth(method=lm,formula=y~x,aes(group=drv))
```
