---
title: "INFSCI 2595 Fall 2021 Homework: 07"
subtitle: "Assigned October 29, 2021; Due: November 4, 2021"
author: Kelin He
date: "Submission time: November 4, 2021 at 11:00PM EST"
output: html_document
---
~/Documents/Machine Learning/Assignment/He_Kelin_HW_08/Kelin_He_HW08.Rmd
...
This homework assignment is focused on model complexity and the influence of the prior **regularization** strength. You will fit non-Bayesian and Bayesian linear models, compare them, and make predictions to visualize the trends. You will use multiple prior *strengths* examining the influence on the coefficient posteriors and on the posterior predictive distributions.  
...
```{r, read_data}
data_url <- 'https://raw.githubusercontent.com/jyurko/INFSCI_2595_Fall_2021/main/HW/08/hw08_prob01.csv'
...
```{r, solution_01a}
df %>% ggplot() + geom_point(mapping = aes(x = x1, y = y),color='red') 
df %>% ggplot() + geom_point(mapping = aes(x = x2, y = y),color='blue')
```
I think there maybe a trend, but hard to observe.
...
coefplot(mod01)
```
x1 and x2 are statistically significant.
...
coefplot(mod02)
```
three features are present in the model. x1, x2,x1:x2 are statistically significant.
...
coefplot(mod03)
```
8 features are present in the model.It seems that x1,x1^2 and x2^2 are significant.
...
coefplot(mod04)
```
It seems no features are significant.
...
broom::glance(mod06)
```
R-squard shows that the model 06 is the best one. however, AIC, BIC  show that model 03 is the best 
...
```{r,2c_1}
pred_lm_01 %>% 
  ggplot(mapping = aes(x=x1))+geom_ribbon(mapping = aes(ymin=pred_lwr,ymax=pred_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=ci_lwr,ymax=ci_upr),fill='grey')+facet_wrap(facets = pred_lm_01$x2)+coord_cartesian(ylim = c(-7,7))+geom_line(mapping = aes(y=pred))
```

```{r,2c_2}
pred_lm_02 %>% 
  ggplot(mapping = aes(x=x1))+geom_ribbon(mapping = aes(ymin=pred_lwr,ymax=pred_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=ci_lwr,ymax=ci_upr),fill='grey')+facet_wrap(facets = pred_lm_02$x2)+coord_cartesian(ylim = c(-7,7))+geom_line(mapping = aes(y=pred))
```
```{r,2c_3}
pred_lm_03 %>% 
  ggplot(mapping = aes(x=x1))+geom_ribbon(mapping = aes(ymin=pred_lwr,ymax=pred_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=ci_lwr,ymax=ci_upr),fill='grey')+facet_wrap(facets = pred_lm_03$x2)+coord_cartesian(ylim = c(-7,7))+geom_line(mapping = aes(y=pred))
```
```{r,2c_4}
pred_lm_04 %>% 
  ggplot(mapping = aes(x=x1))+geom_ribbon(mapping = aes(ymin=pred_lwr,ymax=pred_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=ci_lwr,ymax=ci_upr),fill='grey')+facet_wrap(facets = pred_lm_04$x2)+coord_cartesian(ylim = c(-7,7))+geom_line(mapping = aes(y=pred))
```
```{r,2c_5}
pred_lm_05 %>% 
  ggplot(mapping = aes(x=x1))+geom_ribbon(mapping = aes(ymin=pred_lwr,ymax=pred_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=ci_lwr,ymax=ci_upr),fill='grey')+facet_wrap(facets = pred_lm_05$x2)+coord_cartesian(ylim = c(-7,7))+geom_line(mapping = aes(y=pred))
```
```{r,2c_6}
pred_lm_06 %>% 
  ggplot(mapping = aes(x=x1))+geom_ribbon(mapping = aes(ymin=pred_lwr,ymax=pred_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=ci_lwr,ymax=ci_upr),fill='grey')+facet_wrap(facets = pred_lm_06$x2)+coord_cartesian(ylim = c(-7,7))+geom_line(mapping = aes(y=pred))
```
...
I feel that the predictions are consistent with the model performance rankings based on AIC/BIC
the defining characteristic of the models considered to be the worst are many features within a model.
...
```{r, solution_03c}
laplace_03_weak <- my_laplace(rep(0,10),lm_logpost,info_03_weak)
laplace_06_weak <- my_laplace(rep(0,66), lm_logpost,info_06_weak)
```
...
```{r, solution_03d_a}
viz_post_coefs(laplace_03_weak$mode[1:9],sqrt(diag(laplace_03_weak$var_matrix))[1:9],colnames(X03))
```


```{r, solution_03d_b}
viz_post_coefs(laplace_06_weak$mode[1:65],sqrt(diag(laplace_06_weak$var_matrix))[1:65],colnames(X06))
```
...
BF_weak<-exp(laplace_03_weak$log_evidence)/exp(laplace_06_weak$log_evidence)
BF_weak
```
Since the result of bayes factor is greater than 1, mod3 is better than mod6
...
```{r, solution_03f_b}
laplace_03_strong<-my_laplace(rep(0,10),lm_logpost,info_03_strong)
laplace_06_strong<-my_laplace(rep(0,66),lm_logpost,info_06_strong)
```
...
```{r, solution_03g}
### add more code chunks if you like
viz_post_coefs(laplace_03_strong$mode[1:9],sqrt(diag(laplace_03_strong$var_matrix))[1:9],colnames(X03))
viz_post_coefs(laplace_06_strong$mode[1:65],sqrt(diag(laplace_06_strong$var_matrix))[1:65],colnames(X06))
```
...
```{r, solution_03h_b}
laplace_03_very_strong<-my_laplace(rep(0,10),lm_logpost,info_03_very_strong)
laplace_06_very_strong<-my_laplace(rep(0,66),lm_logpost,info_06_very_strong)
```
...
```{r, solution_03i}
viz_post_coefs(laplace_03_very_strong$mode[1:9],sqrt(diag(laplace_03_very_strong$var_matrix))[1:9],colnames(X03))
viz_post_coefs(laplace_06_very_strong$mode[1:65],sqrt(diag(laplace_06_very_strong$var_matrix))[1:65],colnames(X06))
```
...
With more strong prior, the distributions of coefficient posterior become narrower.
...
BF_very_strong
```
In the strong prior case,the result of Bayes factors is greater than 1,the better model is mod3.
In the very strong prior case, the result of Bayes factors is smaller than 1, the better model is mod6. Therefore, I think that the setting of prior matters.
...
#a<-generate_lm_post_samples(laplace_03_weak,ncol(X03_test),5000)
#b<-make_post_lm_pred(X03_test,a)
...
post_pred_summary_viz_03_weak %>% 
  left_join(viz_grid %>% tibble::rowid_to_column("pred_id"),
            by = 'pred_id')%>% 
  ggplot(mapping = aes(x=x1))+geom_ribbon(mapping = aes(ymin=y_lwr,ymax=y_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=mu_lwr,ymax=mu_upr),fill='grey')+facet_wrap(facets = viz_grid$x2)+coord_cartesian(ylim = c(-7,7))+geom_line(mapping = aes(y=mu_avg))
```
...
```{r, solution_04e}
knots <- make_splines_training_knots(12,viz_grid,'x1')
X06_test <- model.matrix(~ splines::ns(x1, 12,knots=knots)*(x2+ I(x2^2) + I(x2^3) + I(x2^4) ) , data =viz_grid)
...
As the prior standard deviation decreased, the posterior predictions confidence interval and predictive interval both become narrower. This behavior is consistent with the behavior of the posterior coefficients.
...
```
both models' intercepts and x^2 are not turned off. except from values, final tuned models are same in terms of the composition of coefficients.
