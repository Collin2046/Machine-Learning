...
```{r, solution_01a}
prob_01_df %>% ggplot(mapping = aes(x=x)) + geom_line(mapping = aes(y=f)) + geom_point(mapping = aes(y=f)) 
```
...
Since there are two hidden units, 4 beta parameters are necessary. In the output side, the intercept $\alpha$ plus two slopes. Therefore, 3 elements are necessary for output side.
The first column of beta_matrix is the parameters for unit1, the second column is the beta parameters for the hidden unit2.
...
$$
\mathbf{H} = \mathbf{logit}^\left(-1\right) ( \mathbf{X} \times \mathbf{B})=\frac{\exp(\mathbf{X} \times \mathbf{B})}{1+\exp(\mathbf{X} \times \mathbf{B})}
$$
...
nnet_hidden_2a <- calc_hidden_units(X01,params_01_2a$beta_matrix,func)
```
...
The slope parameters controls the increase or decrease trend of hidden linear predictors
...
The middle part of the sin function is fit well by the neural network  
...
nnet_hidden_2b <- calc_hidden_units(X01,params_01_2b$beta_matrix,func)
...
-The result is not consistent with the trend in problem 1, the trend of h_01 changes, it decreases instead of increase.
-I think the change maybe caused by the parameter in parameter of beta, the [2,1] in beta matrix in problem 1 is positive,but the [2,1] in beta matrix in problem 2 is negative, causing hidden linear predictor decreases.
-Therefore, I think the figure draw by new parameters might be somewhat different.
...
The difference between two models is very small, but the second model is fit well to sin wave than the first model.
...
```{r,2c_a}
params_01_2a$beta_matrix
params_01_2b$beta_matrix
```
The slope(positive or negative) influences the trends of linear predictors, but the neural network behavior can not be explained only be slope since the linear predictors still need to experienced non-linear transformation.
...
nnet_hidden_5a <- calc_hidden_units(X01, params_02_5a$beta_matrix, func)

nnet_hidden_5b <- calc_hidden_units(X01, params_02_5b$beta_matrix, func)
```
...
viz_hidden_trend_wrt_x(nnet_hidden_5b$H , 
                       ...
                       "hidden non-linear units")
```
No matter how big the predictor values are. After non-linear transformation, these values are limited in 0-1.
...
results_5hidden %>% 
  ...
  theme_bw()
```
The two model with 5 hidden units fit sin wave very well, and the difference between the two is very small. Nonetheless, the first model nnet_fa fits the sin wave more accurate compared to the the second model nnet_fb.
...
```{r}
mean(abs(prob_01_df$f- results_2hidden$nnet_fa))
mean(abs(prob_01_df$f- results_2hidden$nnet_fb))
mean(abs(prob_01_df$f- results_5hidden$nnet_fa))
mean(abs(prob_01_df$f- results_5hidden$nnet_fb))
```
-based on calculation, the first model of 5 hidden units fits the sin wave well than other three.
-I think the the numbers of units in a model are controlling the neural network's complexity. 
...
The estimate for the slope is 1.
...
```{r,keeptest}
params_02_5a$beta_matrix
params_02_5a$alpha_vector
```
2 unknown parameters in the linear model.
16 unknown parameters in the neural network with 5 hidden units.
...
I think because this is just a simple fit of sin(x) , and when comes to some complex data set, the neural network can be more accurate than lm().
...
```{r, solution_03a}
prob_03_lf %>% ggplot(mapping = aes(x=value))+geom_point(mapping = aes(y=y))+facet_wrap(~name)
```
...
my_neuralnet_sse <- function(theta, my_info)
{
  ...
  a0 <- theta[total_num_betas + 1]# output layer bias??????
  aw <- theta[(total_num_betas + 2):length(theta)]# output layer weights?????
...
They are not same. Acoording to this day's lecture. I think they achieve different local modes.
...
The second and fourth are both better than other two since they have same smaller SSE value.
...
train_1layer_nnet_sse <- function(num_hidden, transform_func, X, y)
{
  ...
  my_info_list$num_params <- my_info_list$num_hidden * ncol(info_three_units$design_matrix) + my_info_list$num_hidden +1# total number of hidden and output layer parameters
...
```
Based on the mathematics calculation, it seems models optim_fit_24_a is the better model since it has the smaller SSE value than other models.
...
{
  Bmat <- matrix(theta[1:(ncol(X)*num_hidden)], nrow = ncol(X), byrow = FALSE)
  eta <- X %*% Bmat
  value_units <- transform_func(eta)
  alpha_0 <- theta[(ncol(X)*num_hidden)+1]
  alpha_rem <- theta[((ncol(X)*num_hidden)+2):length(theta)]
  final_fit <- alpha_0 + value_units %*% alpha_rem
  return ((t(final_fit-y) %*% (final_fit-y)) / nrow(X))
}
```
...
```{r,solu_5c}
MSE_3c <- assess_nnet_mse(optim_fit_3_c$par,3,tanh,Xtest_03,prob_03_test_df$y)
MSE_3d <- assess_nnet_mse(optim_fit_3_d$par,3,tanh,Xtest_03,prob_03_test_df$y)
MSE_6a <- assess_nnet_mse(optim_fit_6_a$par,6,tanh,Xtest_03,prob_03_test_df$y)
MSE_6b <- assess_nnet_mse(optim_fit_6_b$par,6,tanh,Xtest_03,prob_03_test_df$y)
MSE_12a <- assess_nnet_mse(optim_fit_12_a$par,12,tanh,Xtest_03,prob_03_test_df$y)
MSE_12b <- assess_nnet_mse(optim_fit_12_b$par,12,tanh,Xtest_03,prob_03_test_df$y)
MSE_24a <- assess_nnet_mse(optim_fit_24_a$par,24,tanh,Xtest_03,prob_03_test_df$y)
MSE_24b <- assess_nnet_mse(optim_fit_24_a$par,24,tanh,Xtest_03,prob_03_test_df$y)
MSE_3c; MSE_3d; MSE_6a; MSE_6b; MSE_12a; MSE_12b; MSE_24a; MSE_24b
```
...
model optim_fit_12_a is the best model according the result of SSE value.