...
```{r,2c_1}
pred_lm_01 %>% 
  ggplot(mapping = aes(x=x1))+geom_line(mapping = aes(y=pred))+geom_ribbon(mapping = aes(ymin=pred_lwr,ymax=pred_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=ci_lwr,ymax=ci_upr),fill='grey')+facet_wrap(facets = pred_lm_01$x2)+coord_cartesian(ylim = c(-7,7))
```

```{r,2c_2}
pred_lm_02 %>% 
  ggplot(mapping = aes(x=x1))+geom_line(mapping = aes(y=pred))+geom_ribbon(mapping = aes(ymin=pred_lwr,ymax=pred_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=ci_lwr,ymax=ci_upr),fill='grey')+facet_wrap(facets = pred_lm_02$x2)+coord_cartesian(ylim = c(-7,7))
```
```{r,2c_3}
pred_lm_03 %>% 
  ggplot(mapping = aes(x=x1))+geom_line(mapping = aes(y=pred))+geom_ribbon(mapping = aes(ymin=pred_lwr,ymax=pred_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=ci_lwr,ymax=ci_upr),fill='grey')+facet_wrap(facets = pred_lm_03$x2)+coord_cartesian(ylim = c(-7,7))
```
```{r,2c_4}
pred_lm_04 %>% 
  ggplot(mapping = aes(x=x1))+geom_line(mapping = aes(y=pred))+geom_ribbon(mapping = aes(ymin=pred_lwr,ymax=pred_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=ci_lwr,ymax=ci_upr),fill='grey')+facet_wrap(facets = pred_lm_04$x2)+coord_cartesian(ylim = c(-7,7))
```
```{r,2c_5}
pred_lm_05 %>% 
  ggplot(mapping = aes(x=x1))+geom_line(mapping = aes(y=pred))+geom_ribbon(mapping = aes(ymin=pred_lwr,ymax=pred_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=ci_lwr,ymax=ci_upr),fill='grey')+facet_wrap(facets = pred_lm_05$x2)+coord_cartesian(ylim = c(-7,7))
```
```{r,2c_6}
pred_lm_06 %>% 
  ggplot(mapping = aes(x=x1))+geom_line(mapping = aes(y=pred))+geom_ribbon(mapping = aes(ymin=pred_lwr,ymax=pred_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=ci_lwr,ymax=ci_upr),fill='grey')+facet_wrap(facets = pred_lm_06$x2)+coord_cartesian(ylim = c(-7,7))
```
...
With more strong prior, the distribution of posterior mean become very narrow
...
BF_very_strong
```
In the very strong prior case, the result of Bayes factors is small than 1, the better model is mod6. Therefore, the setting of prior matters.
...
post_pred_summary_viz_03_very_strong <- summarize_lm_pred_from_laplace(laplace_03_strong,X03_test,5000)
```
...
post_pred_summary_viz_03_weak %>% 
  left_join(viz_grid %>% tibble::rowid_to_column("pred_id"),
            by = 'pred_id')%>% 
  ggplot(mapping = aes(x=x1))+geom_line(mapping = aes(y=mu_avg))+geom_ribbon(mapping = aes(ymin=y_lwr,ymax=y_upr),fill='orange')+geom_ribbon(mapping = aes(ymin=mu_lwr,ymax=mu_upr),fill='grey')+facet_wrap(facets = viz_grid$x2)+coord_cartesian(ylim = c(-7,7))
```
...
```{r, solution_04e}
X06_test <- model.matrix(~ splines::ns(x1, 12)*(x2+ I(x2^2) + I(x2^3) + I(x2^4) ) , data =viz_grid)
