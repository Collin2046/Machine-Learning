...
subtitle: "Assigned October 21, 2021; Due: October 28, 2021"
author: Kelin He
date: "Submission time: October 28, 2021 at 11:00PM EST"
...
url_01 <- "https://raw.githubusercontent.com/jyurko/INFSCI_2595_Fall_2021/main/HW/07/train_01.csv"
...
```{r, solution_01a}
train_01 %>% ggplot() + geom_point(mapping = aes(x=x,y=y))+geom_smooth(mapping = aes(x=x,y=y),method = 'lm')
```
...
laplace_01 <- my_laplace(rep(0,3),lm_logpost,info_01)
```
...
```{r, solution_01e_c}
post_samples_01 %>% ggplot()+geom_histogram(mapping = aes(x=beta_01),bins = 55)
positive <- post_samples_01 %>% filter(beta_01>0)
positive %>% count(beta_01)
```
the probability that the slope is positive is zero.
...
head_3 <-post_samples_01 %>% head(3)
pred_3 <- make_post_lm_pred(Xmat_01,head_3)
### error of the first posterior sample
error_01_post_01 <- pred_3$Umat[,1]-train_01$y

### error of the second posterior sample
error_01_post_02 <- pred_3$Umat[,2]-train_01$y

### error of the third posterior sample
error_01_post_03 <- pred_3$Umat[,3]-train_01$y
```

I think mean trend is more representative since predicted response valves are also near the mean trend value. 
...
They are not same since they have different beta0 and beta1
...
dimention of absE01mat is 21x2500

```{r, solution_02e, eval=TRUE}
absE01mat<- abs(post_pred_samples_01$Umat-c(train_01$y))
### dimensions?
dim(absE01mat)
```
...
I will use colMeans() to calculate  
...
MAE_01[3]
```
According to checking result, they are as same as result obtained in 2d
...
I think because every set of beta0 and beta1 result in different predicted y, which causes the uncertainty. 
...
url_02_train <- "https://raw.githubusercontent.com/jyurko/INFSCI_2595_Fall_2021/main/HW/07/train_02.csv"

train_02 <- readr::read_csv(url_02_train, col_names = TRUE)

url_02_test <- "https://raw.githubusercontent.com/jyurko/INFSCI_2595_Fall_2021/main/HW/07/test_02.csv"
...
```{r solution_03a}
train_02 %>% ggplot()+geom_point(mapping = aes(x=x,y=y),color='red')+geom_point(data=test_02,mapping = aes(x=x,y=y),color='blue')
```
...
150 rows in training matrices and 50 rows in test metrices.
...
manage_spline_fit <- function(Xtrain, logpost_func, my_settings)
{
  ...
  init_beta <- rnorm(ncol(my_settings$design_matrix),0,1)
...
all_spline_mae_results%>%
  ggplot(mapping = aes(x=as.factor(J),y=mae))+geom_boxplot(mapping=aes(fill=dataset))+scale_fill_brewer(palette = "Set1")
```
...
all_spline_mae_results%>%filter(J>3)%>%
  ggplot(mapping = aes(x=J,y=mae))+stat_summary(geom='line',fun='median')+facet_wrap(facets = 'dataset')
```

It seems that models with high degrees overfit the training data.
...
for (i in 1:25)
{
  A[i]<-exp(spline_evidence[i])
}
denominator=sum(A)

for (i in 1:25)
{
  weight[i] <- exp(spline_evidence[i])/denominator
}
spline_weights <- weight
...
based on the visualization, the model with 7 degree is the best model.
...
The conclusions are not same since based on the visualization of 3e, the best model can not be the model with 7 degree.The conclusions, therefore, are not same. According to definition of Evidence-based assessment, this method executes more penalty to high degree model. Therefore, high degree models are ruled out by this method. And it prefers relatively simple model which is more suitable for the generalization of test set.
