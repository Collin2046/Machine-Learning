---
title: "INFSCI 2595 Fall 2021 Homework: 06"
subtitle: "Assigned October 7, 2021; Due: October 14, 2021"
author: Kelin He
date: "Submission time: October 14, 2021 at 11:00PM EST"
...
```{r, read_in_data}
hw06_data_url <- "https://raw.githubusercontent.com/jyurko/INFSCI_2595_Fall_2021/main/HW/06/hw06_data.csv"
...
{
  vec_3<-c(unknown_1,unknown_2)
  logpost_func(vec_3,logpost_info)
}
```
...
The mode is not as same as sample average and sample standard deviation. According to the figure, the uncertainty of mu is symmetric.However, the uncertainty of sigma is not symmetric.
...
$$
$$
\frac{\partial^2}{\partial\mu^2}\log[p\left(\mu \mid \mathbf{x}, \sigma\right)] \propto
\frac{\partial}{\partial\mu}\left(-\frac{N\mu}{\sigma^2}-\frac{\mu-\mu_0}{\tau_0^2}\right)
$$
$$
\frac{\partial^2}{\partial\mu^2}\log[p\left(\mu \mid \mathbf{x}, \sigma\right)] \propto
\left(-\frac{N}{\sigma^2}-\frac{1}{\tau_0^2}\right)
$$
...
the second derivative of this posterior mode is always smaller than 0.Therefore, the first derivative continue goes down. sothe maximum log-posterior density instead of the minimum density can be found.
...
$$
m_{N}=\frac{\frac{\mu_0}{\tau_0^2}+\frac{N\bar{x}}{\sigma^2}}{{\frac{N}{\sigma^2}}+\frac{1}{\tau_o^2}}
$$
$$
s_N=\sqrt\frac{\tau_0^2\sigma^2}{\sigma^2+N\tau_0^2}
$$
### 2d)
...
in the second-order Taylor series, the gradient vector of the log-posterior with the respect to each parameter of interest, evaluated at the posterior mode by the defination is equal to 0
...
by using change-of-variables transformation from $\sigma=exp(\varphi)$to$ φ=log(\sigma)$,It can make the bound unlimited,which is necessary if we decide to use laplace approximation.
...
$$
p\left( \mu , \varphi\mid \mathbf{x} \right) \propto \prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \times \mathrm{normal}\left(\mu \mid \mu_0, \tau_0\right)\times \mathrm{Exp}\left(g^{-1}( \varphi) \mid \lambda=0.5\right)\times\left( \left| \frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) \right| \right)
$$
...
$$
Write the log of the derivative adjument in an equation block.  
$$
\log\frac{\partial}{\partial\varphi}g^{-1}( \varphi)=\log\left(\exp(\varphi)\right)=\varphi
$$
### 3d)
...
my_cv_logpost <- function(unknowns, my_info)
{
  ...
  log_deriv_adjust <- log(exp(unknowns[2]))
...
map_res_02$par
```
optimal parameter values and Hessian matrices are nearly same.function gradients are not same.
...
sd_mu
sd_varphi <- sqrt(laplace_result$var_matrix[4])
sd_varphi
...
pnorm(q=mean(hw06_df$x),mean=laplace_result$mode[1],sd=sqrt(laplace_result$var_matrix[1,1]))
```
...
```{r,apply}
res<-exp(mean(post_samples$varphi))
res
```
posterior on sigma
```{r,pos_sigma}
mean_sigma<-mean(post_samples$sigma)
mean_sigma
```
they are 2.984029 and 3.068196. I think it is nearly same
...
``` 
posterior distribution on $\sigma$ does not look Gaussian
...
sigma_mean<-mean(post_samples$sigma)
sigma_sd<-sd(post_samples$sigma)
prb=1-pnorm(q=4,mean=sigma_mean,sd=sigma_sd)
prb
```