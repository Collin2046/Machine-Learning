---
title: "INFSCI 2595 Fall 2021 Homework: 05"
subtitle: "Assigned September 30, 2021; Due: October 7, 2021"
author: Kelin He
date: "Submission time: October 7, 2021 at 11:00PM EST"
...
To help out this company, we will assume that the part performance can be represented by a Gaussian likelihood, given the mean performance $\mu$ and the noise $\sigma$. We will assume the observation of each experiment, $x_{n}$, is conditionally independent given those hyperparameters. The joint distribution between $N$ observations can therefore be factored into the product of $N$ separate likelihoods:  
...
**Based on the interval suggested by the engineers, solve for the $\mu_{0}$ and $\tau_{0}$ hyperparameters for the Gaussian prior on the mean, $\mu$.**  
...
**Based on your result in Problem 1a), calculate the 25th and 75th quantiles associated with the prior Gaussian on $\mu$. The interval between these two quantiles contains what fraction of the probabilty mass?**  
...
```
The interval contains 50% fraction of the probability mass.
### 1c)
...
Most of the engineers feel confident that the process standard deviation, $\sigma$, is known. They believe that $\sigma = 5$ units. We will now use this value of $\sigma$ to estimate the required number of observations to achieve a target posterior standard deviation on the mean performance, $\mu$.  
...
A small contingent of engineers feel that the others are being overly optimistic. This group of engineers feel that the process standard deviation is $\sigma = 15$ units, and they do not feel comfortable specifying such a tight prior on $\mu$. They are worried that a large number of tests will be needed to achieve $\tau_{N} = 3$ units.  
...
$$
p\left(\mu \mid \mathbf{x}, \sigma\right) \propto \prod_{n=1}^{N}exp\left(-\frac{1}{2\sigma^2}(x_n-\mu)^2\right)*exp\left(-\frac{1}{2\tau_0^2}(\mu-\mu_0)\right)
$$
$$
\log [p\left(\mu \mid \mathbf{x}, \sigma\right)] \propto \sum_{n=1}^{N}\log exp\left(-\frac{1}{2\sigma^2}(x_n-\mu)^2\right)+\log exp \left(-\frac{1}{2\tau_0^2}(\mu-\mu_0)^2\right)
$$
$$
\log [p\left(\mu \mid \mathbf{x}, \sigma\right)] \propto -\frac{1}{2\sigma^2} \sum_{n=1}^{N}(x_n-\mu)^2-\frac{1}{2\tau_0^2}(\mu-\mu_0)^2
$$

### 2b)

**Derive the derivative of the un-normalized log-posterior with respect to $\mu$. Your final answer should be written in terms of the sufficient statistic, the sample average $\bar{x}$.**  

#### SOLUTION

$$
\frac{\partial}{\partial\mu}\log[p\left(\mu \mid \mathbf{x}, \sigma\right)] \propto\frac{\partial}{\partial\mu}\left(-\frac{1}{2\sigma^2} \sum_{n=1}^{N}(x_n-\mu)^2\right)-\frac{\partial}{\partial\mu}\left(\frac{1}{2\tau_0^2}(\mu-\mu_0)^2\right)
$$

$$
\frac{\partial}{\partial\mu}\log[p\left(\mu \mid \mathbf{x}, \sigma\right)] \propto \frac{1}{\sigma^2}\left(\sum_{n=1}^{N}(x_n)-N\mu\right)-\frac{1}{\tau_0^2}(\mu-\mu_0)
$$
$$
\frac{\partial}{\partial\mu}\log[p\left(\mu \mid \mathbf{x}, \sigma\right)] \propto \frac{1}{\sigma^2}N(\bar{x}-\mu)-\frac{1}{\tau_0^2}(\mu-\mu_0)
$$
...
$$
\frac{1}{\sigma^2}N(\bar{x}-\mu_{MAP})-\frac{1}{\tau_0^2}(\mu_{MAP}-\mu_0)=o
$$
$$
\tau_0^2N(\bar{x}-\mu_{MAP})=\sigma^2(\mu_{MAP}-\mu_0)
$$

$$
\tau_0^2N\bar{x}+\sigma^2\mu_0=\tau_0^2N\mu_{MAP}+\sigma^2\mu_{MAP}
$$
$$
\mu_{MAP}=\frac{\tau_0^2N\bar{x}+\sigma^2\mu_0}{\tau_0^2N+\sigma^2}
$$
...
**Your answer in Problem 2c) should be equivalent to an expression provided in lecture. Which expression is it the same as? Why is your result for the posterior mode the same as that expression? Think in terms of the type of distribution we know the posterior to be given the problem formulation.**  

#### SOLUTION

$$
\mu_{MAP}=\frac{\tau_0^2N\bar{x}+\sigma^2\mu_0}{\tau_0^2N+\sigma^2}=\frac{\frac{\mu_0}{\tau_0^2}+\frac{N\bar{x}}{\sigma^2}}{{\frac{N}{\sigma^2}}+\frac{1}{\tau_o^2}}
$$
...
We will continue to assume all observations are conditionally independent given the parameters. The likelihood therefore factors into the product of $N$ normal likelihoods. Thus, the posterior is proportional to:  
...
You will now start working with the un-normalized log-posterior for unknown $\sigma$.  
...
$$
\log p \left(\sigma \mid \mathbf{x}, \mu\right) \propto \sum_{n=1}^{N}\left(\log \left[\frac{1}{\sigma} exp\left(-\frac{1}{2\sigma^2} (x_n-\mu)^2\right)\right]\right)+\log\left(\exp \left(-\lambda \sigma \right)\right)
$$
$$
\log p \left(\sigma \mid \mathbf{x}, \mu\right) \propto N\log\frac{1}{\sigma} -\frac{1}{2\sigma^2} \sum_{n=1}^{N}(x_n-\mu)^2-\lambda\sigma
$$
...
$$
\log p \left(\sigma \mid \mathbf{x}, \mu\right) \propto N\log\left(\frac{1}{\sqrt{2\pi\sigma^2}}\right)-\frac{1}{2\sigma^2}vN-\lambda\sigma
$$
after making use of v,the result does not depend on individual observation.
### 3c)
...
{
  result=N*log(1/sigma)-(v*N)/(2*sigma^2)-lambda*sigma
  return(result)
}
...
$$
$$
\frac{\partial}{\partial\varphi}=\exp(\varphi)
$$
...
result_2 %>% ggplot(mapping = aes(x=x,y=sin(x)))+geom_line()
```
...
para <- list(N=21,v=4.2,lambda=0.75)
log_post_sigma_unnorm_list <- function(sigma,list_para)
{
  N=list_para$N
  v=list_para$v
  lambda=list_para$lambda
  result=N*log(1/sigma)-(v*N)/(2*sigma^2)-lambda*sigma
  return(result)
}
opt_sine_02 <- optim( par=3,
                      fn=log_post_sigma_unnorm_list,
                     gr = NULL,
                     para,
                     method = "Brent",
                     lower = 0.5, upper = 4.5,
...
why<-optim(
   par=3,
                      fn=log_post_sigma_unnorm,
                     gr = NULL,N=21,v=4.2,lambda=0.75,
...
tibble::tibble(
  ...
  ggplot(mapping = aes(x = x, 
                       y = my_func(x,my_constants)))+geom_line()+geom_vline(xintercept = opt_result$par,color="red")
```
...
some_arg<-list(N=21,v=4.2,lambda=0.75)
log_post_varphi_unnorm_list <- function(varphi,list )
{
  N=list$N
  v=list$v
  lambda=list$lambda
  ...
  # calculate the unnormalized log-posterior on sigma
  log_post <- log_post_sigma_unnorm(sigma,N,v,0.75)
  
  # account for the derivative adjustment
  derivative=log_post+varphi
  return (derivative)
  
}
varphi_opt_a <- optim(par= -0.5 ,
                    fn=log_post_varphi_unnorm_list,
                    gr = NULL,
                    some_arg,
                    method = "BFGS",
                    hessian = TRUE,
...
var_log %>% ggplot(mapping = aes(x=varphi,y=log_post))+geom_line()+geom_vline(xintercept = varphi_opt_b$par,color="red")
```