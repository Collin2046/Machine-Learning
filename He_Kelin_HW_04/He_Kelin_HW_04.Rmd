---
title: "INFSCI 2595 Fall 2021 Homework: 04"
subtitle: "Assigned September 23, 2021; Due: September 30, 2021"
author: Kelin He
date: "Submission time: September 30, 2021 at 11:00PM EST"
...
$$ 
\log \left( p \left( H \mid AB, \mu \right) \right) \propto \log \left(\mu^H(1-\mu)^\left(AB-H\right)\right)
$$
...
result %>% ggplot(mapping = aes(x=mu,y=log_lik)) + geom_line()+geom_vline(xintercept=player_hits/player_atbats)



```

The MLE is corresponds to the vertex of the curve
...
{
  # your code here
  return(log(choose(trials,events)*prob^(events)*(1-prob)^(trials-events)))
}
```
...
MLE correspond to the same location as it did with the un-normalized log-likelihood.
...
exp(1)^(log_binom_pmf(189,602,0.3))
...
coin_probs <- dbinom(x=c(0:5),11,0.25,log=FALSE)
coin_probs
length(coin_probs)
```

The first elemet in the coin_probs represents that there is no coin flipped as head in 11 trials.
...
```{r, solution_02g}
coin_probs_01<- dbinom(x=c(175:195),602,0.3,log=FALSE)
sum(coin_probs_01)
coin_probs_02<- dbinom(x=c(165:205),602,0.3,log=FALSE)
sum(coin_probs_02)
```
...
$$
\log \left( p \left( m \mid N, \mu \right) \right) \propto \log \left(\mu^m(1-\mu)^\left(N-m\right)\right)
$$
...
$$ 
\log \left( p \left( \mu \mid a, b \right) \right) \propto \log \left(\mu^\left(a-1\right)(1-\mu)^\left(b-1\right)\right)
$$
...
$$ 
\log \left(\mu^m(1-\mu)^\left(N-m\right)\right) + \log \left(\mu^\left(a-1\right)(1-\mu)^\left(b-1\right)\right)=  \log \left(\mu^m\right)+\log \left(1-\mu)^\left(N-m\right)\right)+ \log\left(\mu^\left(a-1\right)\right)+\log \left(1-\mu^\left(b-1\right)\right)
$$
$$
=\log\left(\mu^\left(m+a-1\right)\right)+\log \left((1-\mu)^\left(N-m+b-1\right)\right)=\log\left(\mu^\left(m+a-1\right)(1-\mu)^\left(N-m+b-1\right)\right)
$$
$$
a_{new}=m+a
$$
...
$$ 
\frac{\partial}{\partial\mu}\log\left( \mu^{a_{new} -1} \left( 1-\mu\right)^{b_{new}-1}\right) = \frac{\partial}{\partial\mu}\log \left(\mu^{a_{new}-1}\right) + \frac{\partial}{\partial\mu}\log\left( 1-\mu\right)^{b_{new}-1} 
$$


$$
= \left(a_{new}-1 \right)\frac{\partial}{\partial\mu}\log \left(\mu\right) +\left(b_{new} -1 \right) \frac{\partial}{\partial\mu}\log\left( 1-\mu\right)
$$
$$
=\frac{\left(a_{new}-1 \right)}{\mu} - \frac{\left(b_{new}-1 \right)}{1-\mu}
$$
...
result <- tibble::tibble(
  ...
  mutate(vague_mu=dbeta(mu,1,1))

result %>% ggplot(mapping = aes(x=mu,y=vague_mu))+geom_line()
```
...
result<-tibble::tibble(
  ...
  mutate(inform_mu=dbeta(mu,4.28,48.63))
result %>% ggplot(mapping = aes(x=mu,y=inform_mu))+geom_line()
```
...
```{r, solution_04b_a}
hp_small_vague_a_new <- m_small+a_vague
hp_small_vague_b_new <- N_small-m_small+b_vague

hp_small_inf_a_new <- m_small+a_inform
hp_small_inf_b_new <- N_small-m_small+b_inform

hp_medium_vague_a_new <- m_medium+a_vague
hp_medium_vague_b_new <- N_medium-m_medium+b_vague

hp_medium_inf_a_new <-m_medium+a_inform
hp_medium_inf_b_new <-N_medium-m_medium+b_inform

hp_big_vague_a_new <- m_big+a_vague
hp_big_vague_b_new <- N_big-m_big+b_vague

hp_big_inf_a_new <- m_big+a_inform
hp_big_inf_b_new <- N_big-m_big+b_inform

hp_a<- c(m_small+a_vague,m_medium+a_vague,m_big+a_vague,m_small+a_inform,m_medium+a_inform,m_big+a_inform)
hp_b<-c(N_small-m_small+b_vague,N_medium-m_medium+b_vague,N_big-m_big+b_vague,N_small-m_small+b_inform,N_medium-m_medium+b_inform,N_big-m_big+b_inform)



result_summary<-data.frame(hp_a,hp_b)
result_summary
...
```{r, solution_04c_a}
final<- result_summary %>% mutate('5th'=qbeta(0.05,hp_a,hp_b),'95th'= qbeta(0.95,hp_a,hp_b),
mode = (hp_a-1) / (hp_a+hp_b-2),mean = hp_a/(hp_a+hp_b))
final
```
...
#### SOLUTION
compared using vague prior and small size data, informative prior and big size data make the 90% credible interval of posterior narrow, and in both prior, with data increase, the difference between mode and mean become small.
