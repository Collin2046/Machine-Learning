---
title: "INFSCI 2595 Fall 2021 Homework: 02"
subtitle: "Assigned September 8, 2021; Due: September 15, 2021"
author: Kelin He
date: "Submission time: September 15, 2021 at 11:00PM EST"
...
high_noise_github_file <- "https://raw.githubusercontent.com/jyurko/INFSCI_2595_Fall_2021/main/HW/02/prob_high_noise_dataset.csv"
...
RMSE is about to calculate the squared root of MSE, which is related accuracy of predicted data.
...
### linear relationship
set.seed(2001)
mod_high_1 <- train(y~x,data=prob_high_noise,method="lm",trControl=my_ctrl,metric=my_metric)
```

```{r, solution_01e_b, eval=TRUE}
### quadratic relationship
set.seed(2001)
mod_high_2 <- train(y~x+I(x^2),data=prob_high_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_01e_c, eval=TRUE}
### cubic relationship
set.seed(2001)
mod_high_3 <- train(y~x+I(x^2)+I(x^3),data=prob_high_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_01e_d, eval=TRUE}
### 4th order
set.seed(2001)
mod_high_4 <- train(y~x+I(x^2)+I(x^3)+I(x^4),data=prob_high_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_01e_e, eval=TRUE}
### 5th order
set.seed(2001)
mod_high_5 <- train(y~x+I(x^2)+I(x^3)+I(x^4)+I(x^5),data=prob_high_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_01e_f, eval=TRUE}
### 6th order
set.seed(2001)
mod_high_6 <- train(y~x+I(x^2)+I(x^3)+I(x^4)+I(x^5)+I(x^6),data=prob_high_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_01e_g, eval=TRUE}
### 7th order
set.seed(2001)
mod_high_7 <- train(y~x+I(x^2)+I(x^3)+I(x^4)+I(x^5)+I(x^6)+I(x^7),data=prob_high_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_01e_h, eval=TRUE}
### 8th order
set.seed(2001)
mod_high_8 <- train(y~x+I(x^2)+I(x^3)+I(x^4)+I(x^5)+I(x^6)+I(x^7)+I(x^8),data=prob_high_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_01e_i, eval=TRUE}
### 9th order
set.seed(2001)
mod_high_9 <- train(y~x+I(x^2)+I(x^3)+I(x^4)+I(x^5)+I(x^6)+I(x^7)+I(x^8)+I(x^9),data=prob_high_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```
...
According to the two figures, The model 5 is the best model. 
...
low_noise_github_file <- "https://raw.githubusercontent.com/jyurko/INFSCI_2595_Fall_2021/main/HW/02/prob_low_noise_dataset.csv"
...
How does it compare?  
The trend of data in low_noise_github_file is more obvious.
...
### linear relationship
set.seed(2001)
mod_low_1 <- train(y~x,data=prob_low_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_02b_b, eval=TRUE}
### quadratic relationship
set.seed(2001)
mod_low_2 <- train(y~x+I(x^2),data=prob_low_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_02b_c, eval=TRUE}
### cubic relationship
set.seed(2001)
mod_low_3 <- train(y~x+I(x^2)+I(x^3),data=prob_low_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_02b_d, eval=TRUE}
### 4th order
set.seed(2001)
mod_low_4 <- train(y~x+I(x^2)+I(x^3)+I(x^4),data=prob_low_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_02b_e, eval=TRUE}
### 5th order
set.seed(2001)
mod_low_5 <- train(y~x+I(x^2)+I(x^3)+I(x^4)+I(x^5),data=prob_low_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_02b_f, eval=TRUE}
### 6th order
set.seed(2001)
mod_low_6 <- train(y~x+I(x^2)+I(x^3)+I(x^4)+I(x^5)+I(x^6),data=prob_low_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_02b_g, eval=TRUE}
### 7th order
set.seed(2001)
mod_low_7 <- train(y~x+I(x^2)+I(x^3)+I(x^4)+I(x^5)+I(x^6)+I(x^7),data=prob_low_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_02b_h, eval=TRUE}
### 8th order
set.seed(2001)
mod_low_8 <- train(y~x+I(x^2)+I(x^3)+I(x^4)+I(x^5)+I(x^6)+I(x^7)+I(x^8),data=prob_low_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```

```{r, solution_02b_i, eval=TRUE}
### 9th order
set.seed(2001)
mod_low_9 <- train(y~x+I(x^2)+I(x^3)+I(x^4)+I(x^5)+I(x^6)+I(x^7)+I(x^8)+I(x^9),data=prob_low_noise,method="lm",metric=my_metric,trControl=my_ctrl)
```
...
The model 5 is the best model. The best model is as same as the best model of high level noise result.
...
In my opinion, the top 3 models for high noise are 5-4-6,and the top 3 models for low noise are 5-6-7. Therefore, I guess that for data with not specific and ambiguous trend, models with low complexity will be more suitable.
...
binary_class_data_url <- "https://raw.githubusercontent.com/jyurko/INFSCI_2595_Fall_2021/main/HW/02/hw02_binary_class.csv"
...
```{r, solution_03d}
model_pred_df %>% ggplot()+geom_point(mapping=aes(x=x,y=y),size=3.5,alpha=0.5)
```
...
```{r, solution_03f}
model_pred_df %>% ggplot()+geom_point(mapping=aes(x=x,y=y),size=3.5,alpha=0.5)+geom_line(mapping=aes(x=x,y=pred_prob),color="red",size=3.5,alpha=0.5)
```
...
The observed binary response does not follow the model predicted probability.
...
```{r, solution_4b}
matrixA=model_class_0.5 %>% count(pred_class,obs_class)
Accuracy0.5=(matrixA[1,3]+matrixA[4,3])/(matrixA[1,3]+matrixA[4,3]+matrixA[2,3]+matrixA[3,3])
Accuracy0.5
```
...
False-Positives is 6,True-Negatives is 53.
...
```{r, solution_04d}
matrixA<-model_class_0.5 %>% count(pred_class,obs_class)
sensitivity0.5=matrixA[1,3]/(matrixA[1,3]+matrixA[3,3])
sensitivity0.5
FPR=1-(matrixA[4,3]/(matrixA[2,3]+matrixA[4,3]))
FPR
```
...
```{r, solution_05a}
matrixB <- model_class_0.7 %>% count(pred_class,obs_class)
Accuracy0.7=(matrixB[1,3]+matrixB[4,3])/(matrixB[1,3]+matrixB[4,3]+matrixB[2,3]+matrixB[3,3])
Accuracy0.7
```
...
```{r, solution_05b}
sensitivity0.7=matrixB[1,3]/(matrixB[1,3]+matrixB[3,3])
specificity0.7=matrixB[4,3]/(matrixB[2,3]+matrixB[4,3])
sensitivity0.7
specificity0.7
```
...
```{r, solution_05c}
matrixC <- model_class_0.3 %>% count(pred_class,obs_class)
Accuracy0.3=(matrixC[1,3]+matrixC[4,3])/(matrixC[1,3]+matrixC[4,3]+matrixC[2,3]+matrixC[3,3])
Accuracy0.3
```
...
```{r, solution_05d}
sensitivity0.3=matrixC[1,3]/(matrixC[1,3]+matrixC[3,3])
specificity0.3=matrixC[4,3]/(matrixC[2,3]+matrixC[4,3])
sensitivity0.3
specificity0.3
```
...
```{r, solution_05e}
specificity0.5 = matrixA[4,3]/(matrixA[2,3]+matrixA[4,3])

Sensitivity <- c(sensitivity0.3[1,1], sensitivity0.5[1,1], sensitivity0.7[1,1])
FPR <- c(1-specificity0.3[1,1], 1-specificity0.5[1,1], 1-specificity0.7[1,1])
Threshold <- c(0.3, 0.5, 0.7)
DATA_ROC <- tibble(Sensitivity, Threshold, FPR)

ggplot(DATA_ROC, aes(x=FPR,y=Sensitivity,color=Threshold))+geom_point()+geom_abline(slope=1, intercept=0)+geom_step()+coord_equal(xlim=c(0,1), ylim=c(0,1))
```
