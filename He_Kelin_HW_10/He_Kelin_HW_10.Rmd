...
$$
y_{n}\mid \mu_{n}  \sim Bernoulli\left(y_{n}\mid \mu_{n}\right)
$$
$$
\mu_{n} = logit_{}^{-1} \left( \eta_{n}\right)
$$
$$
...
Xmat_spline7 <- model.matrix(~splines::ns(x,7),data=train_df)
```
...
```{r, solution_01d_b}
up_linear <- laplace_linear$mode+2*sqrt(diag(laplace_linear$var_matrix))
low_linear <- laplace_linear$mode-2*sqrt(diag(laplace_linear$var_matrix))
up_linear
low_linear
```
...
```{r, solution_01d_c}
up_cubic <- laplace_cubic$mode+2*sqrt(diag(laplace_cubic$var_matrix))
low_cubic <- laplace_cubic$mode-2*sqrt(diag(laplace_cubic$var_matrix))
up_cubic
low_cubic
```
...
weight_splines
```
Based on the result of weight. cubic model is the better model.
...
generate_glm_post_samples <- function(mvn_result, num_samples)
{
  ...
  beta_samples <-MASS::mvrnorm(n = num_samples,
                mu = mvn_result$mode, Sigma = mvn_result$var_matrix)
...
$$
\frac{\partial}{\partial \beta_0} \left( \log \left( p \left( \boldsymbol{\beta} \mid 0, b \right) \right)  \right) = \frac{\partial}{\partial \beta_0}\left(\log(\sqrt{2\pi}b)^\left(-1\right)-\frac{\beta_0^2}{2b^2}\right)=-\frac{\beta_0}{b^2}
$$
...
$$
\frac{\partial}{\partial \beta_1} \left( \log \left( p \left( \boldsymbol{\beta} \mid 0, b \right) \right)  \right) = \frac{\partial}{\partial \beta_1}\left(\log(\sqrt{2\pi}b)^\left(-1\right)-\frac{\beta_1^2}{2b^2}\right)=-\frac{\beta_1}{b^2}
$$
...
$$
 \frac{\partial L_n}{\partial \mu_n} = \frac{y_n}{\mu_n} - \frac{1-y_n}{1-\mu_n} = \frac{y_n - \mu_n}{\mu_n - \mu_n^{2}}
$$
$$
\frac{\partial \mu_n}{\partial \eta_n} = \mu_n-\mu_n^{2} 
$$
$$
...
$$ 
\frac{\partial}{\partial \beta_0} \left( \log \left( p \left( \mathbf{y} \mid \boldsymbol{\beta}, \mathbf{X} \right) \right)  \right)=\frac{y_n - \mu_n}{\mu_n - \mu_n^{2}} \times \left(\mu_n-\mu_n^{2}\right) \times 1=y_n - \mu_n 
$$
...
$$ 
\frac{\partial}{\partial \beta_0} \left( \log \left( p \left( \boldsymbol{\beta} \mid \mathbf{y}, \mathbf{X} \right) \right) \right)=\frac{\partial}{\partial \beta_0} \left( \log \left( p \left( \mathbf{y} \mid \boldsymbol{\beta}, \mathbf{X} \right) \right) \right) + \frac{\partial}{\partial \beta_0} \left( \log \left( p \left( \boldsymbol{\beta} \mid 0, b \right) \right) \right) \
$$
$$
\frac{\partial}{\partial \beta_0} \left( \log \left( p \left( \boldsymbol{\beta} \mid \mathbf{y}, \mathbf{X} \right) \right) \right) \propto y_n - \mu_n -\frac{\beta_0}{b^2} 
$$
...
$$ 
\frac{\partial}{\partial \beta_1} \left( \log \left( p \left( \mathbf{y} \mid \boldsymbol{\beta}, \mathbf{X} \right) \right) \right)=\frac{\partial L_n}{\partial \mu_n}\frac{\partial \mu_n}{\partial \eta_n}\frac{\partial \eta_n}{\partial \beta_1}=\frac{y_n - \mu_n}{\mu_n - \mu_n^{2}} \times \left(\mu_n-\mu_n^{2}\right) \times x_n=（y_n - \mu_n ）\times x_n
$$
...
$$ 
\frac{\partial}{\partial \beta_1} \left( \log \left( p \left( \boldsymbol{\beta} \mid \mathbf{y}, \mathbf{X} \right) \right) \right)=（y_n - \mu_n ）\times x_n-\frac{\beta_1}{b^2}
$$
...
with prior, the gradient needs to minus $\frac{\beta}{\sigma^2}$, and the big the $beta$ is, the small the gradient will be.
...
```{r, solution_05a}
train_small%>% ggplot()+geom_point(mapping = aes(x=x,y=y))+geom_vline(xintercept = -0.6)
```
Yes, there are still infinite lines to draw to separate these points.
...
train_linear_design <- model.matrix(y~x,data=train_small)
info_small_inform <- list(
  yobs = train_small$y,
  design_matrix = train_linear_design,
  mu_beta = 0,
  tau_beta = 2.5
)
...
small_laplace_from_inform <- my_laplace(rep(0,ncol(train_linear_design)),logistic_logpost,info_small_inform)
```


```{r, solution_05c_b, eval=TRUE}
small_laplace_from_weak <- my_laplace(rep(0,ncol(train_linear_design)),logistic_logpost,info_small_weak)
```
...
```
The posterior modes in weak prior are smaller than the one with inform prior, and the mode of weak one is negatively greater than inform one.
...
small_laplace_from_weak$var_matrix %>% cov2cor()
```
```{r,solution5d_1}
corrplot(cov2cor(small_laplace_from_inform$var_matrix),type="upper")
```
```{r,solution5d_2}
corrplot(small_laplace_from_weak$var_matrix %>% cov2cor(),type="upper")
```
...
when use informative prior, the posterior correlation is smaller compared to using very weak prior.
...
```{r, solution_05f}
mod_small_mle <- glm(y~x, data=train_small,family=binomial(link = logit))
